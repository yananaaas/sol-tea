...
- **Web Components для всіх компонентів** — за зразком `components/sol-nav.js`: кожен компонент → окремий JS-файл у папці `components/`, підключається як кастомний елемент на всіх відповідних сторінках.
- **Джерело правди для вигляду:**
  - `sol-design-system.html` — еталон для тих компонентів, що там повністю відповідають сайту
  - Готові сторінки (`sol-tea.html`, `sol-shop.html` тощо) — джерело для Футера та інших компонентів, функціональність яких в DS обмежена або показова
