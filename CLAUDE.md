...
<<<<<<< Updated upstream
### Пріоритет 5 — База даних і адмін-панель (окремий чат)
Рішення прийнято: Supabase + sol-admin.html
- **Фаза 1** — Supabase і дані: створити проект, таблиці, мігрувати товари
- **Фаза 2** — Сайт читає товари з Supabase (замість JS-масиву)
- **Фаза 3** — sol-admin.html: каталог (CRUD), замовлення, відгуки, підписки
- **Фаза 4** — Форми на сайті → Supabase (checkout, підписка, відгук)

### Пріоритет 6 — Код і архітектура (окремий чат)
=======
### Пріоритет 4 — Код і UX (окремий чат)
>>>>>>> Stashed changes
...
<<<<<<< Updated upstream
### Пріоритет 7 — Тестування і кейс (окремий чат)
=======
### Пріоритет 5 — База даних і адмін-панель (окремий чат)
Рішення прийнято: **Supabase** як база даних + власна адмін-панель `sol-admin.html`.

**Фаза 1 — Supabase і дані:**
- Створити проект у Supabase
- Спроектувати таблиці: `products`, `orders`, `subscriptions`, `reviews`
- Мігрувати дані товарів із JS-масивів у Supabase
- Сайт читає товари через Supabase REST API (замість хардкод масиву)

**Фаза 2 — Адмін-панель:**
- `sol-admin.html` — захищена сторінка (email + пароль через Supabase Auth)
- Екран каталогу: список товарів, кнопки редагування / видалення / додавання
- Форма товару: всі поля (назва, ціна, опис, категорія, регіон, граматура, тощо)
...
**Фаза 3 — Форми на сайті → Supabase:**
- Форма замовлення (sol-checkout.html) → пише в таблицю `orders`
- Підписка у футері → пише в `subscriptions`
- Форма відгуку (sol-product.html) → пише в `reviews` зі статусом "pending"
