...
- **Web Components для всіх компонентів з `sol-design-system.html`** — за зразком `components/sol-nav.js`: кожен компонент → окремий JS-файл у папці `components/`, підключається як кастомний елемент на всіх відповідних сторінках.
- `sol-design-system.html` — еталон і джерело правди для вигляду. Самі оновлення DS — після того як усі сторінки готові (окремий таск, див. нижче).
- Що НЕ виносити в компонент: Футер (складна логіка підписки), компоненти що в DS лише для показу і не повністю функціонують як на сайті.

### Пріоритет 2 — Контент (окремий чат)
- **`products.md`** — каталог усіх товарів у форматі Markdown: назва, категорія, регіон, опис, ціна, параметри заварювання (temp, time, amount, steeps), граматура. Джерело правди для PRODUCTS у JS, адмінки і БД.
...
- **Code review сайту** — після того як about і головна готові, до адмінки
...
### Пріоритет 9 — Мануальне тестування (окремий чат)
- **Функціональне тестування всіх сторінок** — десктоп і мобайл: перевірити кожну інтерактивну дію (кошик, вподобані, пошук, фільтри, пагінація, акордеон, форми, навігація). Фіксувати і одразу виправляти баги.

### Пріоритет 10 — Оновлення дизайн-системи (окремий чат)
- **`sol-design-system.html` — актуалізація** — після того як усі сторінки і компоненти готові: привести DS у відповідність до фінального стану сайту, додати нові компоненти якщо з'явились.

### Пріоритет 11 — Кейс (окремий чат)
